...
IN_PATH = Path("data/raw/BTCUSDT_1h.csv")
OUT_DIR = Path("data/intermediate")
OUT_PATH = OUT_DIR / "btc_1h_features.csv"


def add_return_and_vol_features(df: pd.DataFrame) -> pd.DataFrame:
    # 收益率
...
def add_trend_and_structure_features(df: pd.DataFrame) -> pd.DataFrame:
...
def add_rsi_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    # 1) RSI 斜率
...
def main():
    ...
    df = add_return_and_vol_features(df)
    df = add_trend_and_structure_features(df)
