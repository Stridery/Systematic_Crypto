...
class LogisticTrainer:
    ...
    def __init__(
        ...
        if model_path is None:
            ...
            self.model_path = model_dir / f"{timeframe}_p{lookahead_periods}_logistic.pkl"
        else:
    ...
    def load_data(self, train_ratio: float = None, use_strong_only: bool = True):
        ...
        print(f"[train_logistic] Reading data from {self.data_path} ...")
        ...
        print(f"[train_logistic] Total samples: {n_total}")
        print(f"[train_logistic] Train: {n_train}, Val: {n_total - n_train}")
        print(f"[train_logistic] Feature cols ({len(feature_cols)}): {feature_cols}")
        ...
        if use_strong_only:
            ...
            print(f"[train_logistic] Using strong-only train samples: {mask.sum()} / {len(mask)}")
    ...
    def train(self):
        ...
        log_model.feature_cols = feature_cols
        log_model.train_ratio = self.train_ratio

        print("[train_logistic] Training logistic regression model ...")
        log_model.fit(X_train, y_train, sample_weight=weights_train)

        # ========= 在训练集上评估 =========
        print("\n[train_logistic] Evaluating on TRAIN set ...")
        train_report, train_cm = log_model.evaluate(X_train, y_train)

        print("\n[train_logistic] Classification report (train):")
        print(train_report)

        print("\n[train_logistic] Confusion matrix (train):")
        print(train_cm)

        # ========= 在验证集上评估 =========
        print("\n[val_logistic] Evaluating on VALIDATION set ...")
        val_report, val_cm = log_model.evaluate(X_val, y_val)

        print("\n[val_logistic] Classification report (val):")
        print(val_report)

        print("\n[val_logistic] Confusion matrix (val):")
        print(val_cm)

        # 保存模型
        log_model.save(self.model_path)
        print(f"\n[train_logistic] Model saved to {self.model_path}")
...
def load_data(train_ratio: float = 0.8, use_strong_only: bool = True):
    ...
    注意：现在返回6个值，包括权重
    """
    trainer = LogisticTrainer(train_ratio=train_ratio)
