...
from models.transformer_model import TransformerClassifier, TransformerNNModel
...
class TransformerTrainer:
    ...
    def __init__(
        ...
        if model_pkl_path is None:
            ...
            self.model_pkl_path = model_dir / f"{timeframe}_p{lookahead_periods}_transformer.pkl"
        else:
    ...
    def train(
        ...
        print(f"[train_transformer] Reading data from {self.data_path} ...")
        ...
        print(f"[train_transformer] Feature cols ({len(feature_cols)}): {feature_cols}")
        
        # 标签（-1/0/1）
        y_all_raw = df[LABEL_COL].astype(int).values
        
        # ---- 4. 按时间切分 train / val（和 LightGBM 一样）----
        n_total = len(df)
        n_train = int(n_total * self.train_ratio)
        print(f"[train_transformer] Total samples: {n_total}")
        print(f"[train_transformer] Train: {n_train}, Val: {n_total - n_train}")
        ...
        # ---- 5. 训练集只用 is_strong == 1 的样本（和 LightGBM 一样）----
        if IS_STRONG_COL in df.columns:
            ...
            print(f"[train_transformer] Using strong-only train samples: {strong_count} / {len(mask)}")
            
            if strong_count == 0:
                print("[train_transformer] WARNING: is_strong==1 has 0 samples in train range, using ALL train rows")
            ...
            print("[train_transformer] WARNING: no is_strong column, using ALL train rows")
            X_train = X_train_full
            y_train_raw = y_train_full_raw
            weights_train = weights_train_full
        ...
        print(f"[train_transformer] input_dim={input_dim}, d_model={d_model}, nhead={nhead}, "
              f"num_layers={num_layers}, dim_feedforward={dim_feedforward}, num_classes={num_classes}")
        print(f"[train_transformer] class_weights={class_weights}")
        print(f"[train_transformer] initial lr={lr}, es_patience={es_patience}")
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if new_lr < prev_lr:
                print(f"[train_transformer] LR reduced: {prev_lr:.6f} -> {new_lr:.6f}")
            ...
            if avg_val_loss < best_val_loss - 1e-4:
                ...
                if bad_epochs >= es_patience:
                    print(f"[train_transformer] Early stopping triggered at epoch {epoch}")
                    break
        ...
        # ---- 10. 打包成 TransformerNNModel 并保存 .pkl ----
        self.model_dir.mkdir(parents=True, exist_ok=True)
        
        model_cpu = model.to("cpu")
        nn_model = TransformerNNModel(
            model=model_cpu,
        ...
        print(f"\n[train_transformer] Transformer NNModel saved to {self.model_pkl_path}")
        print(f"[train_transformer] feature_dim={input_dim}, classes={num_classes}")
...
def train_transformer_model(
    ...
    向后兼容的函数接口
    """
    trainer = TransformerTrainer(train_ratio=train_ratio)
    trainer.train(
        d_model=d_model,
        nhead=nhead,
        num_layers=num_layers,
