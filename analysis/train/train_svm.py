...
class SVMTrainer:
    ...
    def load_data(self, train_ratio: float = None, use_strong_only: bool = True):
        ...
        print(f"[train_svm] Reading data from {self.data_path} ...")
        ...
        print(f"[train_svm] Weight stats: min={weights_all.min():.4f}, max={weights_all.max():.4f}, mean={weights_all.mean():.4f}")
        ...
        print(f"[train_svm] Total samples: {n_total}")
        print(f"[train_svm] Train: {n_train}, Val: {n_total - n_train}")
        print(f"[train_svm] Feature cols ({len(feature_cols)}): {feature_cols}")
        ...
        if use_strong_only:
            ...
            print(f"[train_svm] Using strong-only train samples: {mask.sum()} / {len(mask)}")
    ...
    def train(self):
        ...
        svm_model.train_ratio = self.train_ratio

        print("[train_svm] Training SVM model ...")
        svm_model.fit(X_train, y_train, sample_weight=weights_train)
        ...
        print(f"\n[train_svm] Model saved to {self.model_path}")
...
def load_data(train_ratio: float = 0.8, use_strong_only: bool = True):
    ...
    注意：现在返回6个值，包括权重
    """
    trainer = SVMTrainer(train_ratio=train_ratio)
